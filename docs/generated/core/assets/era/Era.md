...
_Creates tile capturing system instance_




### _createNewProsperity
...
_Creates new prosperity instance_
...
_Creates new workers instance_
...
```solidity
function _createNewResource(struct IRegistry.GameResource gameResource, uint256 eraNumber) internal returns (address)
```
...
```solidity
function _createNewUnits(struct IRegistry.GameUnit gameUnit, uint256 eraNumber) internal returns (address)
```
